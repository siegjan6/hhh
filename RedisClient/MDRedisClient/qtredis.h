...
signals:
    ...
    //void returnMessage();
    //subscribe message
    //void returnGeneralCmd(QStringList&);//pinkie

    void messageRet(QList<QByteArray>& val);

public slots:
    void connectHost(const QString &host, const quint16 port = 6379);
    void disconnectHost();
    void sockethaserror();//pinkie
...
private:
    ...
    //********************* pinkie *********************
    QVector<QtRedis::Reply> m_subscribVector;
    QMutex m_subscribemutex;
    QMutex m_connectedmutex;
    bool m_bconnected;
    bool m_bExit;
