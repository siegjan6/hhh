...
#include <QSettings>
//1. socket重连：
//socket error:断开socket＋重连
//socket is disconnected:socket被动关闭时，重连
//QtRedis对象析构时，不重连
...
QtRedis::QtRedis(QString host, int port, QObject *parent) : QObject(parent)
{
    ...
    connect(m_timer,SIGNAL(timeout()),this,SLOT(checkConnectTimer()));
    m_timer->start(1000);

    connect(reader, SIGNAL(response(QVariant)), this, SLOT(response(QVariant)));
    connect(reader, SIGNAL(subResponse()),this,SLOT(responsesub()));

    connect(reader, SIGNAL(connected()), this, SLOT(slotConnected()));
    connect(reader, SIGNAL(disconnected()), this, SLOT(slotDisconnected()));
    connect(reader, SIGNAL(error(QAbstractSocket::SocketError)), this, SLOT(slotDisconnected()));
}
...
void QtRedis::connectHost(const QString &host, const quint16 port)
{
    ...
    //connect(reader, SIGNAL(error(QAbstractSocket::SocketError)), this, SLOT(sockethaserror()));
    //connect(reader, SIGNAL(connected()), this, SLOT(slotConnected()));
}
...
void QtRedis::sockethaserror()//pinkie
{
    QMutexLocker locker(&m_connectedmutex);
    m_bconnected = false;
    emit disconnected();
}
...
void QtRedis::response(QVariant response)
{
    ...
    if(res.size() > 1)
    {
        for(int i = 1; i<res.size(); i++)
            msgList.append(res.at(i).toUtf8());
        emit messageRet(msgList);
    }
}
//******************************* pinkie *******************************
void QtRedis::responsesub()
{
    /*
    while(reader->responseVector.size() > 0)
    {
        res = reader->responseVector.at(0);

        if(res.size() >= 3)
        {
            Reply reply1;
            reply1.type = "list";
            reply1.message = "message";
            reply1.channel = res[1];
            reply1.value = res[2];
            QMutexLocker locker(&m_subscribemutex);
            m_subscribVector.append(reply1);
        }
        res.clear();
        reader->responseVector.pop_front();
    }
    emit returnMessage();
    */
    QStringList res;
    QList<QByteArray> msgList;
    int nsize = reader->responseVector.size();
    for(int i = 0; i<nsize; i++)
    {
        msgList.clear();
        res = reader->responseVector.at(i);
        if(res.size() >= 3)
        {
            msgList.append(res[1].toUtf8());
            msgList.append(res[2].toUtf8());
            emit messageRet(msgList);
        }
        res.clear();
    }
...
void QtRedis::sendCommand(QByteArray& cmd)
{
    //********************* pinkie ********************
    if(!reader->isConnected())
    {
...
bool QtRedis::isconnected()
{
    bool bConnted = false;
    QMutexLocker locker(&m_connectedmutex);
    bConnted = m_bconnected;

    return bConnted;
}

QtRedis::Reply QtRedis::removeHead()
{
    QtRedis::Reply vtRes;
    QMutexLocker locker(&m_subscribemutex);
    if(m_subscribVector.size() > 0)
    {
       vtRes = m_subscribVector.at(0);
       m_subscribVector.removeAt(0);
    }
    return vtRes;
}

void QtRedis::checkConnectTimer()
{
    if(isconnected() == false && m_bExit == false &&     m_bOpenConnection == true)
    {
        connectHost(host,port);
    }
}
