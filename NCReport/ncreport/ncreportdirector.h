...
/*!
  这个类是NCReport的内部引擎。此NCReportDirector类管理了整个报表的运行过程。
  它的职责是运行数据源，渲染报表。
 */
...
报表内部引擎类
【NCReportDirector类管理整个报告运行过程，负责运行数据和绘制报告】
...
public:
    ...
    bool nextRecord(NCReportDataSource* ds = 0, bool isSamplingPageBreak=false);
