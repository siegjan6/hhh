...
报表XML文件源管理引擎类
【管理来自不同位置或存储单元的定义源，报表定义可以来自数据库，文件或通过互联网（HTTP、FTP）等】
...
//    virtual bool loadXML(NCReportXMLDefReader& reader, QMap<QString, QStringList> &info);
    virtual bool save( NCReportXMLDefWriter& writer);

protected:
    virtual bool loadFromFile( NCReportXMLDefReader& );
//    virtual bool loadFromXmlFile(NCReportXMLDefReader& , QMap<QString, QStringList> &info);
