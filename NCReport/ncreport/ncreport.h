...
报表引擎管理类
【ncreport报表生成引擎类。提供重要的API，以便运行报表】
...
public:
    ...
    IMDDBQueryEngine * dbQueryEngine();
    void setDBQueryEngine(IMDDBQueryEngine * pDBQueryEngine);
signals:
...
public slots:
    ...
    void runReportToPdf(const QString& filename );
    int runReportToPDFWithPassword(const QString &filename, const QString& password = "", const QString& perm = "");
...
private:
    QString m_viewPath;//数据视图路径
    IMDDBQueryEngine * m_pDBQueryEngine; //数据查询引擎
private:
    ...
    QMap<QString,QVariant> m_parameters2change;
private:
    void loadConfig();
    bool reportProcess();
