...
{
    QVariant v = itemData(currentIndex());
...
/*!
    ...
    \sa insertColor()
*/
void QtColorComboBox::setCurrentColor(const QColor &color, bool autoAdd )
{
    bool found = false;
    ...
    if (!found && autoAdd ) {
        addColor(color, tr("User color %1").arg(++numUserColors));
...
void QtColorComboBox::emitActivatedColor(int index)
{
    ...
    if (col.isValid()) {
        ...
        setCurrentColor(lastActivated,false);
        col = lastActivated;
    }
