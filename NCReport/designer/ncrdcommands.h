...
{
public:
    NCRDInsertItemCommand( NCReportItem* item, NCRDSectionScene *scene, const NCReportItem::ObjectType objectType, int typeID, const QPointF& pos, QUndoCommand *parent = 0);
    void undo();
    void redo();
    void setLine( const QLineF& );
    void setRect( const QRectF& );
private:
    NCReportItem::ObjectType m_objectType;
    int m_typeId;
    QPointF m_pos;
    QLineF m_line;
    QRectF m_rect;
...
class NCRDLineItemChangeCommand : public NCRDCommand
{
public:
    NCRDLineItemChangeCommand( NCReportItem* item, NCRDSectionScene *scene, QUndoCommand *parent = 0);
