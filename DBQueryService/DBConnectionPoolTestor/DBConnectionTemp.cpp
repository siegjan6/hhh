...
//rand()在多线程时会输出一样的值
//证据:
//CDBConnection::connectDatabase:connection name = SCADA_DB_TYPE_MYSQL_41.DB Name = netscadatestdb"
//CDBConnection::connectDatabase:connection name = SCADA_DB_TYPE_MYSQL_41.DB Name = netsadadb
...
CDBConnectionTemp::CDBConnectionTemp()
    :m_bOpen(false),
     ...
     m_ExtendFlag(0)
{
    //m_strConnectionName = QString("ns_netscade_db_connection_%1").arg(dbConnCount++);
    //Qt5.2 QAtomicInt不支持++操作符 fetchAndAddOrdered(1)+1==++
    //数据归档模块中的字符串为ns_netscade_db_connection
    //这里采用不同的名称 是为了防止连接名相同 导致QtSQL模块出错
    //待完全替换掉MDDBConnet 在将此类单独抽出 形成一个独立的模块

    m_strConnectionName = QString("ns_netscade_db_query_connection_%1").arg(dbConnCount.fetchAndAddOrdered(1)+1);
}
                                                                            ...
                                                                            const QString &user, const QString &passwd, int port)
{
    ...
    {
        MDLOG_ERROR_TO_LOGGER(QString("CDBConnectionTemp::open:fail, err = %1.q conn = %2").arg(dbConn.lastError().text()).arg(m_strConnectionName));
        return false;
    }
...
bool CDBConnectionTemp::openInner(QSqlDatabase & dbConn, DATABASEPARAM::DATABASE_TYPE type)
{
    if(DATABASEPARAM::DB_TYPE_MYSQL == type)
    {
        ...
        if(!b)
        {
            ...
            MDLOG_ERROR_TO_LOGGER(QString("CDBConnectionTemp::open:fail, err = %1; = %2").arg(dbConn.lastError().text()).arg(connInfoStr));
            dbConn.close();
            return false;
        }
