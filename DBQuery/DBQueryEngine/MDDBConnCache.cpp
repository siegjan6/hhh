...
struct PredictOp
{
    CDataBaseParam * pParam;
    bool operator()(MDDBConnection * pConn)
    {
        if (!pConn)
            return false;
...
struct PredictOp2
{
    bool operator()(MDDBConnection * p)
    {
        return p == pCon;
    }
    MDDBConnection * pCon;
...
//在连接池中获得一个连接
MDDBConnection * MDDBConnCache::PullConnection(CDataBaseParam * pDBConnParam)
{
    if(!pDBConnParam)
        return 0;

    MDDBConnection * pCon = 0;
...
//前提假设:一个连接只能被Push一次
 void MDDBConnCache::PushConnection(MDDBConnection  * conn)
 {
    if(!conn)
        return ;
    ...
    {
        DBQE::WriteInfoLog("MDDBConnCache::PushConnection:cann't find conn in used connection list");
        return ;
    }
    ...
    {
        DBQE::WriteInfoLog("MDDBConnCache::PushConnection:find conn in unused connection list");
        return ;
    }
...
void MDDBConnCache::DestoryConnection(MDDBConnection * pConn)
{
    if(pConn)
     {
