...
void CDBSourceParamWidget::initUI(IMDDataSource *pDataSource, CDataArchivesUI *pParent)
{
    if(NULL == pParent || NULL == pDataSource)
    {
        return;
    }
...
void CDBSourceParamWidget::on_NameEdit_textChanged(const QString &arg1)
{
    ...
    QTreeWidget *pQTreeWidget = m_pDataArchivesUI->m_studioInterface->treeWidget();
...
bool CDBSourceParamWidget::ConnectDB(MDDBConnection & adoConnection, IMDDataBaseParam *DBParam, QString &strError)
{
    if(NULL == DBParam)
    {
    ...
    adoConnection.setHost(DBParam->dbServerIP());
    adoConnection.setDBName(DBParam->dbName());
    adoConnection.setUserName(DBParam->dbUser());
    adoConnection.setPassword(DBParam->dbPassword());
    adoConnection.setPort(DBParam->dbServerPort());
    adoConnection.setDBType(ConvertDBType(DBParam->dbServeType()));

    return adoConnection.open();
    /*
    bool bSuccess = false;
    if(NULL == DBParam)
    {
...
{    
    ...
    {
        strTitle = CMDMultiLanguage::instance()->value(tr("成功"));
        strText = CMDMultiLanguage::instance()->value(tr("连接成功"));
    }
    else
    {
        strTitle = CMDMultiLanguage::instance()->value(tr("失败"));
        strText += CMDMultiLanguage::instance()->value(tr("连接失败"));
        strText += strError;
    }
