...
void CNewDBsourceDialog::initLanguage()
{
...
void CNewDBsourceDialog::on_OkButton_clicked()
{
    ...
    if (strName.isEmpty())
    {
        ...
        message.setText(CMDMultiLanguage::instance()->value(tr("名称不能为空")));
    ...
    if(m_pDataArchives->dataSourceManager()->getDataSource(strName))
    {
        ...
        message.addButton(CMDMultiLanguage::instance()->value(tr("确认")), QMessageBox::YesRole);
        message.setWindowTitle(CMDMultiLanguage::instance()->value(tr("提示")));
        QString strFront = CMDMultiLanguage::instance()->value(tr("对象"));
        QString strBack = CMDMultiLanguage::instance()->value(tr("已存在！"));
