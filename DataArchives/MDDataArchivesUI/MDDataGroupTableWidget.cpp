...
void CMDDataGroupTableWidget::changeLanguage()
{
    initLanguage();
}
...
//    m_pBtnOk = new QPushButton(("确定"), this);
    ...
    m_pBtnSelectVar = new QPushButton(tr("选择变量..."), this);
...
//    m_pBtnCancel->setFixedSize(75, 23);
    ...
    m_pLbTabDescrip = new QLabel(tr("数据组成员"), this);
...
//    m_pHFirstLayout->addWidget(m_pCheckEnable);
    ...
    m_pHThirdLayout->addWidget(m_pBtnSelectVar);
...
void CMDDataGroupTableWidget::initLanguage()
{
...
//    connect(m_pBtnOk, SIGNAL(clicked()), this, SLOT(on_okButton_clicked()));
    connect(m_pBtnSelectVar, SIGNAL(clicked()), this, SLOT(on_selectVarButton_clicked()));
...
void CMDDataGroupTableWidget::on_selectVarButton_clicked()
{
    ...
    for(int i = 0; i < count; ++i)
    {
        ...
        pItem->setDescription(pVarVal->getDescription());
...
void CMDDataGroupTableWidget::on_addButton_clicked()
{
    ...
    IMDDataItem *pItem = NULL;
    QString newName = "new1";
        pItem = m_pDataGrp->getItem(newName);
        if(NULL != pItem)
        {
            return;
        }
...
void CMDDataGroupTableWidget::on_removeButton_clicked()
{
    ...
    if (pItem->isFixed())
    {
        ...
        QString strName = CMDMultiLanguage::instance()->value(tr("为系统属性，不允许删除！"));
