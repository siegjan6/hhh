...
void CMDDataSrcTabWidget::initUI()
{
    ...
    //数据组
    m_pLbDataGrpName = new QLabel(tr("数据组名"), this);
...
void CMDDataSrcTabWidget::initTableView()
{
    ...
    headerList << tr("字段名") << tr("数据组成员")
               << tr("数据类型") << tr("主键") << tr("允许为空");
    m_pTabViewModel->insertColumn(0, headerList.at(0));
    m_pTabViewModel->insertColumn(1, headerList.at(1));
    m_pTabViewModel->insertColumn(2, headerList.at(2));
    m_pTabViewModel->insertColumn(3, headerList.at(3));
    m_pTabViewModel->insertColumn(4, headerList.at(4));
