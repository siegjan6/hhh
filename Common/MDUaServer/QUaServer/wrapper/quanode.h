...
class QUaOffNormalAlarm;
class QUaExclusiveLevelAlarm;
...
class MDUASERVER_EXPORT QUaNode : public QObject
{
...
public:
	...
	virtual QUaOffNormalAlarm* addOffNormalAlarm(
		const QUaQualifiedName& browseName,
		const QUaNodeId& nodeId = QUaNodeId()
	);
    virtual QUaExclusiveLevelAlarm* addExclusiveLevelAlarm(
        const QUaQualifiedName& browseName,
        const QUaNodeId& nodeId = QUaNodeId()
    );
	virtual void removeBaseObject(
		const bool& deleteReferences = true
	);
