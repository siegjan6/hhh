		                      ...
		                      const UA_DataValue    *data)
{
	...
	if (srv->m_currentSession) {
		static const QMetaMethod valueChangedFromClientSignal = QMetaMethod::fromSignal(&QUaBaseVariable::valueChangedFromClient);
		if (var->isSignalConnected(valueChangedFromClientSignal))
		{
			// emit value changed
			emit var->valueChangedFromClient(var->value(), !var->m_bInternalWrite);
		}
...
void QUaBaseVariable::setWriteCallback(const std::function<QVariant()>& writeCallback) {
	UA_ValueCallback callback;
	if (writeCallback)
	{
		callback.onWrite = &QUaBaseVariable::onWrite;
		m_writeCallback = writeCallback;
		m_writeCallbackRunning = false;
	}
	else
	{
