...
void QUaAlarmCondition::setActive(const bool& active, const QString& strMessageAppend/* = QString()*/)
{
	...
	if (active)
	{
		...
		this->setMessage(
            ...
            tr("Alarm %1. Requires Acknowledge.%2").arg(strActiveStateName).arg(m_extMsg) :
            tr("Alarm %1. %2 Requires Acknowledge.%3").arg(strActiveStateName).arg(strMessageAppend).arg(m_extMsg)
		);
	}
	else
	{
        ...
        this->setMessage(strMessage+m_extMsg);
	}
	// trigger event
	this->setTime(time);
