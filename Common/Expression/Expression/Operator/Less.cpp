...
CFunctionBase *CLess::create()
{
    return new CLess();
}

IEmpty *CLess::excute(bool *isok)
{
    if (m_args.length() >2)
    {
        if(isok != NULL)
...
void CLess::pushArg(IEmpty *arg,bool& isok)
{
    if (NULL == arg)
    {
        isok = false;
...
QString CLess::keyWord()
{
    return "<";
}

bool CLess::inHerits(const QString &typeName)
{
    bool b = ("CLess" == typeName);
    if(b == false)
