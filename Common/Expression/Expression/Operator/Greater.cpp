...
CFunctionBase *CGreater::create()
{
    return new CGreater();
}

IEmpty *CGreater::excute(bool *isok)
{
    if (m_args.length() >2)
   {
        if(isok != NULL)
...
void CGreater::pushArg(IEmpty *arg,bool& isok)
{
    if (NULL == arg)
    {
        isok = false;
...
QString CGreater::keyWord()
{
    return ">";
}

bool CGreater::inHerits(const QString &typeName)
{
    bool b = ("CGreater" == typeName);
    if(b == false)
