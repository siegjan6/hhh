...
CFunctionBase *CLOr::create()
{
    return new CLOr();
}

IEmpty *CLOr::excute(bool *isok)
{
    if (m_args.length() >2)
    {
        if(isok != NULL)
       ...
       double d1 = 0;
       bool bOK = false;
       d1 = (arg0==0) ? 0 : ((CVariant*)arg0)->toDouble(&bOK);
       if(bOK)
       {
           a1 = d1;
       }
       ...
       double d2 = 0;
       bOK = false;
       d2 = (arg1==0) ? 0 : ((CVariant*)arg1)->toDouble(&bOK);
       if(bOK)
       {
           a2 = d2;
       }
       ...
       if(bArg1)
        ...
        v->setValue((int)(a1||a2));
       return v;
}

void CLOr::pushArg(IEmpty *arg,bool& isok)
{
    if (NULL == arg)
    {
        isok = false;
...
QString CLOr::keyWord()
{
    return "||";
}

bool CLOr::inHerits(const QString &typeName)
{
    bool b = ("CLOr" == typeName);
    if(b == false)
