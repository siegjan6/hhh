...
CFunctionBase *CAOr::create()
{
    return new CAOr();
}

IEmpty *CAOr::excute(bool *isok)
{
    if (m_args.length() >2)
   {
    if(isok != NULL)
...
void CAOr::pushArg(IEmpty *arg,bool& isok)
{
    if (NULL == arg)
    {
        //"Multiplication";
...
QString CAOr::keyWord()
{
    return "|";
}

bool CAOr::inHerits(const QString &typeName)
{
    bool b = ("CAOr" == typeName);
    if(b == false)
