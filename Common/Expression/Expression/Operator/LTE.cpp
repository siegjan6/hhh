...
CFunctionBase *CLTE::create()
{
    return new CLTE();
}

IEmpty *CLTE::excute(bool *isok)
{
    if (m_args.length() >2)
    {
        if(isok != NULL)
...
void CLTE::pushArg(IEmpty *arg,bool& isok)
{
    if (NULL == arg)
    {
        isok = false;
...
QString CLTE::keyWord()
{
    return "<=";
}

bool CLTE::inHerits(const QString &typeName)
{
    bool b = ("CLTE" == typeName);
    if(b == false)
