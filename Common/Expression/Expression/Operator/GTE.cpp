...
CFunctionBase *CGTE::create()
{
    return new CGTE();
}

IEmpty *CGTE::excute(bool *isok)
{
    if (m_args.length() >2)
   {
        if(isok != NULL)
...
void CGTE::pushArg(IEmpty *arg,bool& isok)
{
    if (NULL == arg)
    {
        isok = false;
...
QString CGTE::keyWord()
{
    return ">=";
}

bool CGTE::inHerits(const QString &typeName)
{
    bool b = ("CGTE" == typeName);
    if(b == false)
