...
CFunctionBase *CLAnd::create()
{
    return new CLAnd();
}

IEmpty *CLAnd::excute(bool *isok)
{
    if (m_args.length() >2)
   {
        if(isok != NULL)
...
void CLAnd::pushArg(IEmpty *arg,bool& isok)
{
    if (NULL == arg)
    {
        isok = false;
...
QString CLAnd::keyWord()
{
    return "&&";
}

bool CLAnd::inHerits(const QString &typeName)
{
    bool b = ("CLAnd" == typeName);
    if(b == false)
