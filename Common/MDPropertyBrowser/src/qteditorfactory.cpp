...
static inline void setPropertySource (QtProperty * &property)
{
    if(property){
        property->setSource(IMDIDECHANGE);
        QSet<QtProperty *> parentItems = property->parentItems();
        if(!parentItems.isEmpty()){
            QSetIterator<QtProperty *> it(parentItems);
            while(it.hasNext()){
                it.next()->setSource(IMDIDECHANGE);
            }
        }
    }
}
...
public:
    ...
    Editor *overCreateEditor(QtProperty *property, QWidget *parent);
...
template <class Editor>
Editor *EditorFactoryPrivate<Editor>::overCreateEditor(QtProperty *property, QWidget *parent)
{
    Editor *editor = new Editor(parent,property->markText());
    initializeEditor(property, editor);
    return editor;
}
...
void QtSpinBoxFactoryPrivate::slotSetValue(int value)
{
    ...
    for (QMap<QSpinBox *, QtProperty *>::ConstIterator itEditor = m_editorToProperty.constBegin(); itEditor !=  ecend; ++itEditor) {
        if (itEditor.key() == object) {
            QtProperty *property = itEditor.value();
            setPropertySource(property); //connlaio
            QtIntPropertyManager *manager = q_ptr->propertyManager(property);
            if (!manager)
                return;
...
public:
    ...
    void slotSetEditorClick(QString value);
};
...
void QtCheckBoxFactoryPrivate::slotSetEditorClick(QString value)
{
    QObject *object = q_ptr->sender();
...
//    QtBoolEdit *editor = d_ptr->createEditor(property, parent);
    QtBoolEdit *editor = d_ptr->overCreateEditor(property,parent);
    ...
    connect(editor,SIGNAL(valueChanged(QString)),this,SLOT(slotSetEditorClick(QString)));
    return editor;
}
...
class QtLineEditFactoryPrivate : public EditorFactoryPrivate<CLineEdit>
{
                ...
                const QString &value)
{
    ...
    QListIterator<CLineEdit *> itEditor( m_createdEditors[property]);
    while (itEditor.hasNext()) {
        CLineEdit *editor = itEditor.next();
...
{
    QObject *object = q_ptr->sender();
    const QMap<CLineEdit *, QtProperty *>::ConstIterator ecend = m_editorToProperty.constEnd();
    for (QMap<CLineEdit *, QtProperty *>::ConstIterator itEditor = m_editorToProperty.constBegin(); itEditor != ecend; ++itEditor)
        if (itEditor.key() == object) {
            QtProperty *property = itEditor.value();
            setPropertySource(property); // connlaio
            QtStringPropertyManager *manager = q_ptr->propertyManager(property);
        ...
        QtProperty *property, QWidget *parent)
{
    ...
    //Connailo
    CLineEdit *editor = d_ptr->createEditor(property, parent);
    ...
    connect(editor,SIGNAL(returnText(const QString &)),
...
public:
    ...
    void slotPropertyChanged(QtProperty *property, int value);
    void slotTextChanged(QtProperty *property, QString text);
    void slotEnumNamesChanged(QtProperty *property, const QStringList &);
    void slotEnumIconsChanged(QtProperty *property, const QMap<int, QIcon> &);
    void slotSetValue(int value);
    void slotSetText(QtProperty *property, QString text);//! mark
};
...
void QtEnumEditorFactoryPrivate::slotTextChanged(QtProperty *property, QString text)
{
    if (!m_createdEditors.contains(property))
        return;
...
void QtEnumEditorFactoryPrivate::slotSetText(QtProperty *property, QString text)
{
//    QObject *object = q_ptr->sender();
//    const  QMap<QComboBox *, QtProperty *>::ConstIterator ecend = m_editorToProperty.constEnd();
//    for (QMap<QComboBox *, QtProperty *>::ConstIterator itEditor = m_editorToProperty.constBegin(); itEditor != ecend; ++itEditor)
//        if (itEditor.key() == object) {
//            QtProperty *property = itEditor.value();
//            setPropertySource(property);
//            QtEnumPropertyManager *manager = q_ptr->propertyManager(property);
//            if (!manager)
//                return;
//            manager->setText(property, text);
//            return;
//        }
    setPropertySource(property);
    QtEnumPropertyManager *manager = q_ptr->propertyManager(property);
...
void QtEnumEditorFactory::connectPropertyManager(QtEnumPropertyManager *manager)
{
    ...
    connect(manager,SIGNAL(textChanged(QtProperty*,QString)),this,SLOT(slotTextChanged(QtProperty *, QString)));
}
...
//    qDebug()<<property->propertyName()<<property->markText();
    if(property->markText().right(2) == "_W")
    {
        editor->setEditable(true); //! mark
        editor->setLineEdit(m_lineEdit);
        m_property = property;
    }
    ...
    connect(m_lineEdit,SIGNAL(editingFinished()),this,SLOT(editFinsh()));
    connect(this,SIGNAL(sendText(QtProperty*,QString)),this,SLOT(slotSetText(QtProperty*,QString)));
    return editor;
}
...
void QtEnumEditorFactory::editFinsh()
{
    QString str = m_lineEdit->text();
    emit sendText(m_property,str);
}
