...
static void emitSignal( PropertyManager *manager,
            void (PropertyManager::*valueChangedSignal)(QtProperty *,ValueChangeParameter),
            void (PropertyManager::*updatePropertySignal)(QtProperty*,ValueChangeParameter),
            QtProperty *property, const Value &value)
{
    switch (property->source()) {
    case IMDIDECHANGE:
        emit (manager->*valueChangedSignal)(property,value);
        emit (manager->*updatePropertySignal)(property,value);
        break;
    case OUTSIDECHANGE:
        emit (manager->*valueChangedSignal)(property,value);
        break;
...
void QtStringPropertyManager::setValue(QtProperty *property, const QString &val)
{
    ...
    //edited by Maori
    emitSignal<const QString &, QString,QtStringPropertyManager>(this,
                  &QtStringPropertyManager::valueChanged,
                  &QtStringPropertyManager::updatePropertyOutside,
                  property,val);
}
...
void QtBoolPropertyManager::setValue(QtProperty *property, bool val)
{
    ...
    emit propertyChanged(property);
    //edited by Maori
    emitSignal<bool, bool,QtBoolPropertyManager>
            (this,
             &QtBoolPropertyManager::valueChanged,
             &QtBoolPropertyManager::updatePropertyOutside,
             property,val);
}
...
void QtLocalePropertyManager::setValue(QtProperty *property, const QLocale &val)
{
    ...
    //edited by Maori
    emitSignal<const QLocale &, QLocale,QtLocalePropertyManager>
            (this,
             &QtLocalePropertyManager::valueChanged,
             &QtLocalePropertyManager::updatePropertyOutside,
             property,val);
//    switch (property->source()) {
//    case IMDIDECHANGE:
...
//        break;
//    case OUTSIDECHANGE:
//        emit updatePropertyOutside(property, QVariant::fromValue(val));
//        // edited by Maori,maybe have bugs.
//    default:
//        break;
//    }
}
...
void QtPointPropertyManager::setValue(QtProperty *property, const QPoint &val)
{
    ...
    //edited by Maori
    emitSignal<const QPoint &, QPoint, QtPointPropertyManager>
            (this,
             &QtPointPropertyManager::valueChanged,
             &QtPointPropertyManager::updatePropertyOutside,
             property,val);
//    switch (property->source()) {
//    case IMDIDECHANGE:
//        emit valueChanged(property, val);
//        break;
//    case OUTSIDECHANGE:
//        emit updatePropertyOutside(property,val);
//        break;
//    default:
//        break;
//    }
...
void QtPointFPropertyManager::setValue(QtProperty *property, const QPointF &val)
{
    ...
    emit propertyChanged(property);
    //edited by Maori
    emitSignal<const QPointF &, QPointF, QtPointFPropertyManager>
            (this,
             &QtPointFPropertyManager::valueChanged,
             &QtPointFPropertyManager::updatePropertyOutside,
             property,val);
//    switch (property->source()) {
//    case IMDIDECHANGE:
//        emit valueChanged(property, val);
...
void QtRectPropertyManager::setValue(QtProperty *property, const QRect &val)
{
    ...
    //edited by Maori
    emitSignal<const QRect &, QRect, QtRectPropertyManager>
            (this,
             &QtRectPropertyManager::valueChanged,
             &QtRectPropertyManager::updatePropertyOutside,
             property,val);
//    switch (property->source()) {
//    case IMDIDECHANGE:
...
//        break;
//    case OUTSIDECHANGE:
//        emit updatePropertyOutside(property,data.val);
//        break;
//    default:
//        break;
//    }
...
{//connlaio
    ...
    emit valueChanged(property,data.val);
    //edited by Maori
//    emitSignal<const QRectF &, QRectF, QtRectFPropertyManager>
//            (this,
//             &QtRectFPropertyManager::valueChanged,
//             &QtRectFPropertyManager::updatePropertyOutside,
//             property,val);
//             property->setSource(IMDIDECHANGE);
}
...
QString QtEnumPropertyManager::valueText(const QtProperty *property) const
{
    ...
    if (v >= 0 && v < data.enumNames.count())
        return data.enumNames.at(v);//! mark
...
void QtEnumPropertyManager::setValue(QtProperty *property, int val)
{
    ...
    emit propertyChanged(property);
    emit enumNamesChanged(property,data.enumNames);//! mark

    //edited by Maori
    emitSignal<int, int, QtEnumPropertyManager>
            (this,
             &QtEnumPropertyManager::valueChanged,
             &QtEnumPropertyManager::updatePropertyOutside,
             property,val);
    switch (property->source()) {
    case IMDIDECHANGE:
        emit valueChanged(property,data.val);
...
void QtEnumPropertyManager::setText(QtProperty *property, QString text)
{
    const QtEnumPropertyManagerPrivate::PropertyValueMap::iterator it = d_ptr->m_values.find(property);
    if (it == d_ptr->m_values.end())
        return;
    ...
    QStringList strList = data.enumNames;
    if(strList.contains(text) || text.isEmpty())
        return;

    data.enumNames.append(text);
    if (data.enumNames.count() > 0)
        data.val = 0;
    data.val = data.enumNames.count()-1;
    it.value() = data;

    setEnumNames(property,strList<<text);
    emit enumNamesChanged(property, data.enumNames);

    emit propertyChanged(property);
    emit valueChanged(property, data.val);
    emitSignal<QString, QString, QtEnumPropertyManager>
            (this,
             &QtEnumPropertyManager::textChanged,
             &QtEnumPropertyManager::updatePropertyOut,
             property,text);
}
...
void QtFlagPropertyManager::setValue(QtProperty *property, int val)
{
    ...
    //edited by Maori
    emitSignal<int , int, QtFlagPropertyManager>
            (this,
             &QtFlagPropertyManager::valueChanged,
             &QtFlagPropertyManager::updatePropertyOutside,
             property,val);
//    emit valueChanged(property, data.val);
}
...
void QtSizePolicyPropertyManager::setValue(QtProperty *property, const QSizePolicy &val)
{
    ...
    //edited by Maori
    emitSignal<const QSizePolicy &, QSizePolicy, QtSizePolicyPropertyManager>
            (this,
             &QtSizePolicyPropertyManager::valueChanged,
             &QtSizePolicyPropertyManager::updatePropertyOutside,
             property,val);
//    switch (property->source()) {
//    case IMDIDECHANGE:
//        emit valueChanged(property,val);
...
void  QtFontPropertyManagerPrivate::slotFontDatabaseChanged()
{
    if (!m_fontDatabaseChangeTimer) {
        ...
        //调用QFontDatabase::addApplicationFont时会触发此函数，并触发valueChanged事件，需要屏蔽
        //aGRw 2016.07.18
...
void QtFontPropertyManager::setValue(QtProperty *property, const QFont &val)
{
    ...
    //edited by Maori
    emitSignal<const QFont &, QFont, QtFontPropertyManager>
            (this,
             &QtFontPropertyManager::valueChanged,
             &QtFontPropertyManager::updatePropertyOutside,
             property,val);
//    emit valueChanged(property, val);
}
...
void QtColorPropertyManager::setValue(QtProperty *property, const QColor &val)
{
    ...
    //edited by Maori
    emitSignal<const QColor &, QColor, QtColorPropertyManager>
            (this,
             &QtColorPropertyManager::valueChanged,
             &QtColorPropertyManager::updatePropertyOutside,
             property,val);
//    switch (property->source()) {
//    case IMDIDECHANGE:
//        emit valueChanged(property,val);
...
void QtCursorPropertyManager::setValue(QtProperty *property, const QCursor &value)
{
    ...
    emit propertyChanged(property);
    //edited by Maori
    emitSignal<const QCursor &, QCursor, QtCursorPropertyManager>
            (this,
             &QtCursorPropertyManager::valueChanged,
             &QtCursorPropertyManager::updatePropertyOutside,
             property,value);
