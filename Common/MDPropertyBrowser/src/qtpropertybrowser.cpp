...
/*!
    Returns the property's  source.

    \sa setSource()
*/
int QtProperty::source() const
{
    return d_ptr->m_source;
}
/*!
    Returns the property's  parent items.
*/
QSet<QtProperty *> QtProperty::parentItems() const
{
...
QString QtProperty::markText() const
{
    return d_ptr->m_markText;
}
/*!
    Sets the property's source to the given \a source.

    \sa source()
*/
void QtProperty::setSource(int source)
{
    if(d_ptr->m_source == source)
        return;
    d_ptr->m_source = source;
}
...
void QtProperty::setMarkText(const QString &text)
{
    if (d_ptr->m_markText == text)
        return;

    d_ptr->m_markText = text;
    propertyChanged();
}
...
*/
QtProperty *QtAbstractPropertyManager::addProperty(const QString &name, const QString &markText)
{
    QtProperty *property = createProperty();
    if (property) {
        property->setPropertyName(name);
        property->setMarkText(markText);
...
public:
    QtBrowserItemPrivate(QtAbstractPropertyBrowser *browser, QtProperty *property, QtBrowserItem *parent)
        : m_browser(browser), m_property(property), m_parent(parent), q_ptr(0),m_isExpanded(false) {}
    ...
    bool m_isExpanded;// Added By Maori .Expanded state
    bool m_isVisible;
};
