...
QtBoolEdit::QtBoolEdit(QWidget *parent,const QString &markText) :
    QWidget(parent),
    m_markText(markText),
    m_checkBox(new QCheckBox(this)),
    m_textVisible(true)
{
    if(m_markText.right(3)!= "BTN")
    {
    ...
    else if(m_markText.right(3) == "BTN")
    {
        ...
        m_pushButton->setSizePolicy(QSizePolicy(QSizePolicy::Fixed, QSizePolicy::Preferred));
        m_pushButton->setText(QLatin1String("..."));
        m_pushButton->setFixedWidth(40);
        layout->setMargin(0);
        layout->setSpacing(0);
        layout->addWidget(m_checkBox);

        layout->addItem(new QSpacerItem(0, 0, QSizePolicy::Expanding, QSizePolicy::Ignored));
        setFocusProxy(m_pushButton);
        setFocusPolicy(m_pushButton->focusPolicy());
