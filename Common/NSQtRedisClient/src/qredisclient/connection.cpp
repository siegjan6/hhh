...
void RedisClient::Connection::setTimeouts(uint connectionTimeout, uint commandExecutionTimeout)
{
    m_config.setTimeouts(connectionTimeout,commandExecutionTimeout);
}
...
{
    //Command cmd(rawCmd, owner, callback, db);//chp
    Command cmd(rawCmd, this,[this](Response r, QString e)
    {
        QList<QByteArray> msg;
        if(r.isMessage())
        {
            QList<QVariant> msg1 = r.getValue().toList();
            for(int i = 1; i<msg1.count(); i++)
                msg.append(msg1.at(i).toByteArray());
        }
        else
        {
            msg.append(r.getValue().toByteArray());
        }
        emit RedisClient::Connection::messageRet(msg);
    },db);
    try {
        this->runCommand(cmd);
    } catch (RedisClient::Connection::Exception& e) {
...
RedisClient::Response RedisClient::Connection::commandSync(const Command& command)
{
    ...
    /*if (r.isEmpty())
        throw Exception("Execution timeout");*///chp
    return r;
}
