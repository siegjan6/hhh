      ...
      use_single(AcusNever), explicit_fillups(""), fillups_enabled(false)
{
    ...
    SendScintilla(SCI_AUTOCSETCASEIMDENSITIVEBEHAVIOUR,
            SC_CASEIMDENSITIVEBEHAVIOUR_IGNORECASE);
...
void QsciScintilla::setFolding(FoldStyle folding, int margin)
{
    ...
    SendScintilla(SCI_SETMARGIMDENSITIVEN, margin, 1);
...
{
    if (line < 0)
        SendScintilla(SCI_MARGIMDETTEXT, line, (const char *)0);
    else
...
void QsciScintilla::setMarginText(int line, const QString &text, int style)
{
    ...
    SendScintilla(SCI_MARGIMDETTEXT, line,
            ScintillaBytesConstData(textAsBytes(text)));

    SendScintilla(SCI_MARGIMDETSTYLE, line, style - style_offset);
}
...
void QsciScintilla::setMarginText(int line, const QList<QsciStyledText> &text)
{
    ...
    SendScintilla(SCI_MARGIMDETSTYLES, line, styles);
...
{
    return SendScintilla(SCI_GETMARGIMDENSITIVEN, margin);
}
...
{
    SendScintilla(SCI_SETMARGIMDENSITIVEN, margin, sens);
}
