...
配方文件界面管理类，以树结构管理配方文件。
来自 houdp ，不做过多注释。

*/

#ifndef MDRECIPEFILENODE_H
#define MDRECIPEFILENODE_H

#include <QTreeWidgetItem>

class CMDRecipeUIImp;

const QString sExtension  = ".rcp";
const QString sRootName = "配方";
const QString sAddFile = "添加配方";
...
const QString sNewFileName = "NewRecipe";
const QString sNodeSign = "Recipe";
...
//节点操作类，用于图形窗体管理
class CMDRecipeFileNode : public QObject
{
    Q_OBJECT
public:
    CMDRecipeFileNode();
...
public:
    ...
    void initNodes(QTreeWidgetItem* root, CMDRecipeUIImp* scriptUI);
    //清空root所在的节点
    void close(QTreeWidgetItem* root);
private:
    void initTreeView(QTreeWidgetItem* root);
    void initRoot(QTreeWidgetItem* root, CMDRecipeUIImp* scriptUI);
    void initMenu();
...
private:
    ...
    CMDRecipeUIImp* getscriptUI(QTreeWidgetItem* node);
...
private:
    ...
    void renameForm(CMDRecipeUIImp* scriptUI, QTreeWidgetItem* node, const QString& oldFullName);
    void renameForms(QTreeWidgetItem *node, const QString& oldFullName);
    void closeForm(CMDRecipeUIImp* scriptUI, QTreeWidgetItem *node);
