...
CMDDevice::CMDDevice()
{
    ...
    //***************************************************************//
    //************************属性框变量-设备参数***********************//
    m_commProtocol_Pro = "MitsubishiFX3";
    m_replyTimeout_Pro = "1000";
    m_retries_Pro = "3";
    m_station_Pro = "0";
    m_protocolFormat_Pro = (MDProtocolFormat_Pro)0;
    m_checkSum_Pro = 0;
    m_overTimer_Pro = "0";
    //***************************************************************//
}
...
void CMDDevice::copy(IDevice *otherDevice)
{
    ...
    this->setCheckSum(((CMDDevice*)otherDevice)->checkSum());
    this->setprotocalFormat(((CMDDevice*)otherDevice)->protocalFormat());
    this->setComOverTimer(((CMDDevice*)otherDevice)->comOverTimer());
 }
...
{
    m_station = station;
    m_station_Pro = QString::number(m_station,10);
}
qint16 CMDDevice::protocalFormat()
{
    return m_nProtocalFormat;
}
void CMDDevice::setprotocalFormat(qint16 format)
{
    m_nProtocalFormat = format;
    m_protocolFormat_Pro = (MDProtocolFormat_Pro)m_nProtocalFormat;
}
bool CMDDevice::checkSum()
{
    return m_bCheckSum;
}
void CMDDevice::setCheckSum(bool checksum)
{
    m_bCheckSum = checksum;
    m_checkSum_Pro = m_bCheckSum;
}
qint16 CMDDevice::comOverTimer()
{
    return m_nComOverTimer;
}
void CMDDevice::setComOverTimer(qint16 overtimer)
{
    m_nComOverTimer = overtimer;
    m_overTimer_Pro = QString::number(m_nComOverTimer,16);
}
...
void CMDDevice::saveConfiguration(QDomDocument &doc,QDomElement &parentElement)
{
    ...
    element = doc.createElement("ProtocalFormat");
    deviceElement.appendChild(element);
    elementText = doc.createTextNode(QString("%1").arg(this->protocalFormat()));
    element.appendChild(elementText);

    element = doc.createElement("CheckSum");
    deviceElement.appendChild(element);
    elementText = doc.createTextNode(QString("%1").arg(this->checkSum()));
    element.appendChild(elementText);

    element = doc.createElement("CommunicationOverTimer");
    deviceElement.appendChild(element);
    QString strComOverTimer = QString::number(this->comOverTimer(),16).toUpper();
    elementText = doc.createTextNode(strComOverTimer);
    element.appendChild(elementText);
...
void CMDDevice::serialize(CSortJsonObject &json)
{
    ...
    json.insertSimpleValue("ProtocalFormat",this->protocalFormatIndexToString(this->protocalFormat()));
    json.insertSimpleValue("CheckSum",QString("%1").arg(this->checkSum()));
    json.insertSimpleValue("CommunicationOverTimer",QString::number(this->comOverTimer(),16).toUpper());
...
void CMDDevice::readConfiguration(QDomElement &domElement)
{
    ...
    while ( !element.isNull() )
    {
        ...
        else if(0==QString::compare(element.tagName(),"ProtocalFormat", Qt::CaseInsensitive))
        {
            this->setprotocalFormat((qint16)element.text().toInt());
        }
        else if(0==QString::compare(element.tagName(),"CheckSum", Qt::CaseInsensitive))
        {
            this->setCheckSum((bool)element.text().toInt());
        }
        else if(0==QString::compare(element.tagName(),"CommunicationOverTimer", Qt::CaseInsensitive))
        {
            bool ok = 0;
            this->setComOverTimer((qint16)element.text().toInt(&ok,16));
        }
...
void CMDDevice::deserialize(const QJsonObject &json)
{
    ...
    this->setCommType(this->commTypeStringToEnum(json.value("CommType").toString("0")));
    this->setCommParameter(this->commParameterStringToIndex(json.value("CommParameter").toString("Com1,9600,7,2,2")));
    ...
    this->setprotocalFormat(this->protocalFormatStringToIndex(json.value("ProtocalFormat").toString("0")));
    this->setCheckSum(json.value("CheckSum").toString("0").toInt());
    bool ok = 0;
    this->setComOverTimer(json.value("CommunicationOverTimer").toString("0").toInt(&ok,16));
...
void CMDDevice::dynamicSetParameter(const QVariantMap &deviceParameterMap)
{
    ...
    for(int i=0;i<commParameterList.count();i++)
    {
        ...
        else if(0==QString::compare(parameterName,"ProtocalFormat", Qt::CaseInsensitive))
        {
            this->setprotocalFormat((qint16)parameterValue.toInt());
        }
        else if(0==QString::compare(parameterName,"CheckSum", Qt::CaseInsensitive))
        {
            this->setCheckSum((bool)parameterValue.toInt());
        }
        else if(0==QString::compare(parameterName,"CommunicationOverTimer", Qt::CaseInsensitive))
        {
            bool ok = 0;
            this->setComOverTimer((qint16)parameterValue.toInt(&ok,16));
        }
...
QString CMDDevice::protocalFormatIndexToString(qint16 index)
{
    QString strprotocalFormat = QString("%1").arg(index);
    if(index == 0)
    {
        strprotocalFormat = "Format1";
    }
    else if(index == 1)
    {
        strprotocalFormat = "Format4";
    }
    return strprotocalFormat;
}
qint16 CMDDevice::protocalFormatStringToIndex(QString str)
{
    qint16 nprotocalFormat = (qint16)str.toInt();
    if(0==QString::compare(str,"Format1",Qt::CaseInsensitive))
    {
        nprotocalFormat = 0;
    }
    else if(0==QString::compare(str,"Format4",Qt::CaseInsensitive))
    {
        nprotocalFormat = 1;
    }
    return nprotocalFormat;
}
//**********************************属性框翻译与提示***************************//
void CMDDevice::initTranslateNameAndPropertyType()
{
    ...
    //!协议格式
    m_translateName.insert(QString("ProtocolFormat"),QString("协议格式"));
    m_propertyTip.insert(QString("ProtocolFormat"),QString("协议格式"));
    //!和校验
    m_translateName.insert(QString("CheckSum"),QString("和校验"));
    m_propertyTip.insert(QString("CheckSum"),QString("和校验"));
    //!通信超时时间
    m_translateName.insert(QString("OverTimer"),QString("通信超时时间"));
    m_propertyTip.insert(QString("OverTimer"),QString("通信超时时间：16进制(0~F)*10ms"));
    //**********************************************************************//
...
//!协议格式
CMDDevice::MDProtocolFormat_Pro CMDDevice::protocolFormat_Pro()
{
    return m_protocolFormat_Pro;
}
void CMDDevice::setProtocolFormat_Pro(CMDDevice::MDProtocolFormat_Pro format)
{
    m_protocolFormat_Pro = format;
    m_nProtocalFormat = (qint16)m_protocolFormat_Pro;
}
//!和校验
bool CMDDevice::checkSum_Pro()
{
    return m_checkSum_Pro;
}
void CMDDevice::setCheckSum_Pro(bool sum)
{
    m_checkSum_Pro = sum;
    m_bCheckSum = m_checkSum_Pro;
}
//!通信超时时间
QString CMDDevice::overTimer_Pro()
{
    return m_overTimer_Pro;
}
void CMDDevice::setOverTimer_Pro(QString timer)
{
    m_overTimer_Pro = timer;
    bool ok = 0;
    m_nComOverTimer = m_overTimer_Pro.toInt(&ok,16);
}
//***********************************************************************//
