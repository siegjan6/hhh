...
public:
      ...
      * \param address:
        *qint16型开始地址
      */
     void setStartAddress(qint32 address);
     /*!
      * \brief startAddress:
        *获取开始地址
      * \return:
      ...
      * \param address:
        *qint16型结束地址
      */
     void setEndAddress(qint32 address);
     /*!
      * \brief endAddress:
        *获取结束地址
      * \return:
...
public:
     ...
     * \return:
        *值获取是否成功.
     */
    bool getDataBufferULongValue(int internalAddress,  ulong &data);
    /*!
...
private:
    ...
    qint32 m_startAddress;      //!开始地址
    qint32 m_endAddress;        //!结束地址
    QString m_strRegisterType;//!字符串类型寄存器
    QString m_strStartAddress;//!字符串类型开始地址
    QString m_strEndAddress;//!字符串类型结束地址
private:
    uint m_packCount;//!分包总数
    uint m_currentPack;//!当前包
    quint8 m_DBWriteArray[2][600];//!每包数据
    int m_currentPackLength[2];
public:
    void setPackBlockWrite(uint allpack);
    uint  packBlockWrite();
    void setCurrentPackBlockWrite(uint currentpack);
    uint  currentPackBlockWrite();
    void cpyTempDBWriteBuffer(unsigned char * pbuffer,uint cpyLength,int nwei);
    quint8 * tempDBWtiteBuffer(int nwei);
    void setCurrentPackLength(int length,uint currentpack);
    int    currentPackLength(uint currentpack);
public:
    /*!
...
public:
    int verifyRegisterFormat(QString strRegister,qint32 &nRegister,int &addressFormat,qint16 &nLength);
    int verifyAddressFormat(QString strAddress,qint32 &nAddress,qint16 addressFormat);
    int verifyLength(qint32 startAddress,qint32 endAddress,qint16 &nLength,qint32 registerType);
    void overallVerify();
};
