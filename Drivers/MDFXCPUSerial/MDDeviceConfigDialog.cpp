...
void CMDDeviceConfigDialog::setDevice(CMDDevice *device)
{
    ...
    if(m_device!=NULL)
    {
        ...
        ui->PLCTypecomboBox->setCurrentIndex(m_device->PLCType());
...
void CMDDeviceConfigDialog::okButton_clicked()
{
    ...
    if(m_device!=NULL)
    {
        ...
        IMDDriverManager *driverManager = (IMDDriverManager*)m_device->parent(MDTreeItemType::DriverManager);
        ...
        else if(ui->communicationTypeBox->currentIndex()== 2)
        {
            ...
            QString  param = QString("%1,%2,%3,,").arg(commModel)
                                                        .arg(ui->serverIPEdit->text())
                                                        .arg(ui->serverPortEdit->text())
                                                        .arg(ui->localPortEdit->text());
            m_device->setCommParameter(param);
        ...
        m_device->setReplyTimeout(ui->replyTimeoutEdit->text().toInt());
        m_device->setRetries(ui->retriesEdit->text().toInt());
        m_device->setPLCType(ui->PLCTypecomboBox->currentIndex());
    }
