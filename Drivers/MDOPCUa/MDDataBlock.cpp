...
static int onSubCallbackFunc(QString entityPath, QVariant value, int status, void* contex)
{
    if(NULL != contex){
        CMDDataBlock* pBlock = (CMDDataBlock*)contex;
        if(pBlock && pBlock->getDataBuffer())
        {
            qDebug() << "status=" << status;
            // 判断品质
            if(status > 0){
                // status == 99 ua掉线
                pBlock->setQuality(MDVariableDataQuality::ConnectDeviceFailed);
                pBlock->refreshVariablesQuality();
            }
            else if(status == 0)
            {
                // status == 0 ua连接正常
                pBlock->setQuality(MDVariableDataQuality::Good);
                pBlock->refreshVariable(entityPath, value);
            }
        }
    }
...
bool CMDDataBlock::start()
{
    ...
    CBaseChannel* channel = getChannel();
    if(channel){
        ICommunicationComponent* com = channel->getCommunicationComponent();
        if(com && com->getCommType() == CommunicationType::OPCUA){
            com->registSubCallback(onSubCallbackFunc, this);
            com->subscribes(nameStringVector());

        }
    }
    bool ret = CBaseDataBlock::start();
    return ret;
}
...
quint8* CMDDataBlock::getDataBuffer()
{
    return m_dataBuffer;
}
...
int CMDDataBlock::refreshVariable(QString varpath, QVariant value)
{
    qDebug() << "refreshVariable(" << varpath <<", " << value << ")";
    QString realVarPath = "OPCUA__."+varpath.replace('/','.');
    IMDRTDBVariable *nsVar = getVariable(realVarPath);
    if (NULL == nsVar)
    {
        return 1;
    }
    ...
    setQuality(MDVariableDataQuality::Good);
    if(nsVar->refreshData(value, timeStamp(), quality(), false))
    {
        changevarList.append(nsVar);
        nsVar->notifyDeviceVariablesChange(changevarList);
    }

    return 0;
}

bool CMDDataBlock::asyncWriteVariable(IMDRTDBVariable *nsVar)
{
    CBaseChannel* channel = getChannel();
    if(channel){
        ICommunicationComponent* com = channel->getCommunicationComponent();
        if(com && com->getCommType() == CommunicationType::OPCUA){
            com->asyncWriteVariable(nsVar->address(), nsVar->originalDataType(), nsVar->originalValue());
        }
    }
    return true;
}

bool CMDDataBlock::asyncWriteVariables(QList<IMDRTDBVariable*>& nsVarList)
{
    CBaseChannel* channel = getChannel();
    if(channel){
        ICommunicationComponent* com = channel->getCommunicationComponent();
        if(com && com->getCommType() == CommunicationType::OPCUA){
            ...
            QList<MDDataType> types;
            QList<QVariant> values;
            foreach(IMDRTDBVariable* nsVar, nsVarList)
            {
                addresses.append(nsVar->address());
                types.append(nsVar->originalDataType());
                values.append(nsVar->originalValue());
            }
            com->asyncWriteVariables(addresses, types, values);
        }
    }
    return true;
}
...
void CMDDataBlock::refreshVariablesQuality()
{
    ...
    for (int varIndex = 0; varIndex < getVariableCount(); varIndex++)
    {
        ...
        {
            lastChangeVar =nsVar;
            nsVar->refreshData(/*QVariant()*/nsVar->currentValue(), timeStamp(), quality(), true);
        }
    }
