...
static bool m_firstSub = true;                //!第一次读命令为sub订阅命令
...
//    else if(m_receiveData[2] == 0xFD)
//    {
//        // sub callback
//        quint16 rcvlength = *(quint16*)&m_receiveData[4];
//        qDebug() << "var change recv length=" << rcvlength;
//        QString::fromLatin1((char*)m_receiveData+9,rcvlength);
//        setReceiveDataLength(0);
//        setCommandResult(ProtocolCommandResult::Right);
//        m_receiveData[2] = 0x00;
//        return getCommandResult();
//    }
    else if(m_receiveData[2] == 0x00)
    {
        setReceiveDataLength(0);
        setCommandResult(ProtocolCommandResult::Right);
        return getCommandResult();
