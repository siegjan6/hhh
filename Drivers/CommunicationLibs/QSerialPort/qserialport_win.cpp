...
bool QSerialPortPrivate::waitForReadyRead(int msecs)
{
    ...
    do {
        ...
        if (!waitAnyEvent(timeoutValue(msecs, stopWatch.elapsed()), &timedOut, &triggeredEvent) || !triggeredEvent) {
            ...
            qDebug("exit waitAnyEvent2");
            if (!timedOut)
                q->setError(decodeSystemError());
            return false;
        }
        qDebug("exit waitAnyEvent");
        if (triggeredEvent == communicationOverlapped.hEvent)
        {
            qDebug("_q_canCompleteCommunication");
            _q_canCompleteCommunication();

        }
        else if (triggeredEvent == readCompletionOverlapped.hEvent) {
            qDebug(QString::number(readBuffer.size()).append("444444444444444444").toLatin1());
            qDebug("_q_canCompleteRead");
            _q_canCompleteRead();
            QString str;
            str.append(QString::number(qint64(readBuffer.size())));
            str.append(" , ");
            str.append(QString::number(currentReadBufferSize));
            str.append(" , ");
            str.append(QString::number(initialReadBufferSize));
            qDebug(str.toLatin1());
            if (qint64(readBuffer.size()) != currentReadBufferSize)
            {
                qDebug("currentReadBufferSize = readBuffer.size()");
...
bool QSerialPortPrivate::waitForBytesWritten(int msecs)
{
    ...
    forever {
        ...
        if (!waitAnyEvent(timeoutValue(msecs, stopWatch.elapsed()), &timedOut, &triggeredEvent) || !triggeredEvent) {
            if (!timedOut)
                q->setError(decodeSystemError());
            qDebug("waitForBytesWritten false");
            return false;
        }
...
bool QSerialPortPrivate::startAsyncRead()
{
    ...
    DWORD numberOfBytesTransferred=0;//lqz
