...
private:
    uint m_startAddressBlockWrite;//!批量写开始地址
    uint m_endAddressBlockWrite;//!批量写结束地址
    uint m_packCount;//!分包总数
    uint m_currentPack;//!当前包
    quint8 m_DBWriteArray[6][256];//!每包数据
public:
    /*!
     * \section:
        * 设置及获取批量写时的信息.
     */
    void setStartAddressBlockWrite(uint startaddress);
    uint  startAddressBlockWrite();
    void setEndAddressBlockWrite(uint endaddress);
    uint  endAddressBlockWrite();
