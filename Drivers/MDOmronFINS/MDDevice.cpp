...
CMDDevice::CMDDevice()
{
    m_commParameter = "Client,127.0.0.1,9600,,";
    setCommType(CommunicationType::UDP);
    ...
    //***************************************************************//
    //************************属性框变量-设备参数***********************//
    m_commProtocol_Pro = "Omron FINS Ethernet";
...
void CMDDevice::serialize(CSortJsonObject& json)
{
    ...
    json.insertSimpleValue("DeviceType",QString("%1").arg(this->deviceType()));
...
void CMDDevice::deserialize(const QJsonObject &json)
{
    ...
    this->setCommParameter(this->commParameterStringToIndex(json.value("CommParameter").toString("Com1,115200,8,0,0")));
...
//!站号
QString CMDDevice::deviceType_Pro()
{
    return m_deviceType_Pro;
}
void CMDDevice::setDeviceType_Pro(QString deviceType)
{
    m_deviceType_Pro = deviceType;
    m_deviceType = (qint16)deviceType.toInt();
}
