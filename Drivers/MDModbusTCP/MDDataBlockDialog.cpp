...
void CMDDataBlockDialog::registerComBox_currentIndexChanged(int registerIndex)
{
    ...
    if(tempRegisterFlag == REGISTER_FORMAT_RIGHT)
    {
        ...
        strPrompt  = CMDMultiLanguage::instance()->value(tr(u8"开始地址(%1进制:地址范围最大长度%2)")).arg(nTempAddressFormat).arg(nTempLength);
        ui->label_6->setText(strPrompt);
        strPrompt  = CMDMultiLanguage::instance()->value(tr(u8"结束地址(%1进制:地址范围最大长度%2)")).arg(nTempAddressFormat).arg(nTempLength);
        ui->label_7->setText(strPrompt);
    }
    else
    {
