    ...
    ui(new Ui::CMDDeviceConfigDialog)
{
    ...
    connect(ui->ModelType,SIGNAL(currentIndexChanged(int)),this,SLOT(ModelType_currentIndexChanged(int)));
...
void CMDDeviceConfigDialog::setDevice(CMDDevice *device)
{
    ...
    if(m_device!=NULL)
    {
        ...
        else if(m_device->commType()== CommunicationType::TCP)
        {
            ...
            if(paramList.size()>4)
            {
                ...
                else
                {
                 ...
                 ui->localIPEdit->setText(paramList.at(3));
        ...
        //********************************PKX*******************************//
        ui->ModelType->setCurrentIndex(m_device->PLCType());

        ui->LocalTSAP->setText(QString("%1").arg(m_device->LocalTSAP()));
         ui->RemoteTSAP->setText(QString("%1").arg(m_device->RemoteTSAP()));
...
void CMDDeviceConfigDialog::okButton_clicked()
{
    if(m_device!=NULL)
    {
        ...
        m_device->setPLCType(ui->ModelType->currentIndex());//PKX
        if(m_device->PLCType() == 0 || m_device->PLCType() == 1)
        {
            m_device->getPropertyBrowse()->setSinglePropertyHide("CPURack",true);
            m_device->getPropertyBrowse()->setSinglePropertyHide("CPUSlot",true);
            m_device->getPropertyBrowse()->setSinglePropertyHide("LocalTSAP",false);
            m_device->getPropertyBrowse()->setSinglePropertyHide("RemoteTSAP",false);
        }
        else if(m_device->PLCType() == 2 || m_device->PLCType() == 3)
        {
            m_device->getPropertyBrowse()->setSinglePropertyHide("CPURack",false);
            m_device->getPropertyBrowse()->setSinglePropertyHide("CPUSlot",false);
            m_device->getPropertyBrowse()->setSinglePropertyHide("LocalTSAP",true);
            m_device->getPropertyBrowse()->setSinglePropertyHide("RemoteTSAP",true);
        }
        m_device->setLocalTSAP(ui->LocalTSAP->text());//PKX
        m_device->setRemoteTSAP(ui->RemoteTSAP->text());//PKX
...
//*************************PKX************************//
void CMDDeviceConfigDialog::ModelType_currentIndexChanged(int index)
{
    if(index == PLC_1200)//1200PLC
    {
        ui->LocalTSAP->setText("10.11");
        ui->RemoteTSAP->setText("3.01");
        ui->label_20->setText("Local TSAP");
        ui->label_21->setText("Remote TSAP");
    }
    else if(index == PLC_1500)
    {
        ui->LocalTSAP->setText("10.11");
        ui->RemoteTSAP->setText("3.01");
        ui->label_20->setText("Local TSAP");
    ...
    else if(index == PLC_300)
    {
        ui->LocalTSAP->setText("0");
        ui->RemoteTSAP->setText("2");
        ui->label_20->setText("CPU Rack(0-7)");
        ui->label_21->setText("CPU Slot(2-31)");
    }
    else if(index== PLC_400)
    {
        ui->LocalTSAP->setText("0");
        ui->RemoteTSAP->setText("2");
        ui->label_20->setText("CPU Rack(0-7)");
