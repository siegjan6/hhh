...
CMDDevice::CMDDevice()
{
    ...
    m_commParameter = "Client,127.0.0.1,5000,,65535";
    m_station= 0;
    ...
    m_tcpOrUdpLocalPort = "65535";
    m_tcpOrUdpRemotePort = "5000";
    //!USB
    m_usbParameter = "";
    //!板卡
    m_boardParameter = "";
    ...
    //***************************************************************//
    //************************属性框变量-设备参数***********************//
    m_commProtocol_Pro = "MitsubishiE71";
    m_replyTimeout_Pro = "1000";
    m_retries_Pro = "3";
    m_station_Pro = "1";
    m_octalOrHex_Pro = (MDXYFormat_Pro)1;//!xy寄存器地址格式
    m_protocolType_Pro = (MDMCProtocol_Pro)0;//!协议类型
    m_netNumber_Pro = "0";//!网络号
    //***************************************************************//
}
CMDDevice::~CMDDevice()
{
...
void CMDDevice::copy(IDevice *otherDevice)
{
    ...
    this->setStation(((CMDDevice*)otherDevice)->station());
    this->setCommType(((CMDDevice*)otherDevice)->commType ());
    this->setCommParameter(((CMDDevice*)otherDevice)->commParameter());
    this->setNetNumber(((CMDDevice*)otherDevice)->netNumber());
    this->setXYRegisterFormat(((CMDDevice*)otherDevice)->xyRegisterFormat());
    this->setProtocalType(((CMDDevice*)otherDevice)->protocalType());
...
{
    m_station = station;
    m_station_Pro = QString::number(m_station,10);//!属性框
}

qint16 CMDDevice::netNumber()
{
    return m_netNumber;
}

void CMDDevice::setNetNumber(qint16 netnumber)
{
    m_netNumber = netnumber;
    m_netNumber_Pro = QString::number(m_netNumber,10);//!属性框
}

bool CMDDevice::xyRegisterFormat()
{
    return m_octalOrHex;
}

void CMDDevice::setXYRegisterFormat(bool format)
{
    m_octalOrHex = format;
    m_octalOrHex_Pro = (MDXYFormat_Pro)m_octalOrHex;//!属性框
}

bool CMDDevice::protocalType()
{
    return m_protocalType;
}

void CMDDevice::setProtocalType(bool protocal)
{
    m_protocalType = protocal;
    m_protocolType_Pro = (MDMCProtocol_Pro)m_protocalType;//!属性框
}
...
void CMDDevice::saveConfiguration(QDomDocument &doc,QDomElement &parentElement)
{
    ...
    element = doc.createElement("NetNumber");
    deviceElement.appendChild(element);
    elementText = doc.createTextNode(QString("%1").arg(this->netNumber()));
    element.appendChild(elementText);

    element = doc.createElement("XYFormat");
    deviceElement.appendChild(element);
    elementText = doc.createTextNode(QString("%1").arg(this->xyRegisterFormat()));
    element.appendChild(elementText);

    element = doc.createElement("ProtocalType");
    deviceElement.appendChild(element);
    elementText = doc.createTextNode(QString("%1").arg(this->protocalType()));
    element.appendChild(elementText);
...
void CMDDevice::serialize(CSortJsonObject &json)
{
    ...
    json.insertSimpleValue("Station",QString("%1").arg(this->station()));
    json.insertSimpleValue("NetNumber",QString("%1").arg(this->netNumber()));
    json.insertSimpleValue("XYFormat",this->XYFormatIndexToString(this->xyRegisterFormat()));
    json.insertSimpleValue("ProtocalType",this->MCProtocalIndexToString(this->protocalType()));
...
void CMDDevice::readConfiguration(QDomElement &domElement)
{
    ...
    while ( !element.isNull() )
    {
        ...
        else if(0==QString::compare(element.tagName(),"NetNumber", Qt::CaseInsensitive))
        {
            this->setNetNumber((qint16)element.text().toInt());
        }
        else if(0==QString::compare(element.tagName(),"XYFormat", Qt::CaseInsensitive))
        {
            this->setXYRegisterFormat((bool)element.text().toInt());
        }
        else if(0==QString::compare(element.tagName(),"ProtocalType", Qt::CaseInsensitive))
        {
            this->setProtocalType((bool)element.text().toInt());
        }
...
void CMDDevice::deserialize(const QJsonObject &json)
{
    ...
    this->setCommParameter(this->commParameterStringToIndex(json.value("CommParameter").toString("Client,127.0.0.1,5000,,65535")));
    this->setReplyTimeout_Pro(json.value("ReplyTimeout").toString("1000"));
    this->setRetries_Pro(json.value("Retries").toString("3"));
    this->setStation(json.value("Station").toString("0").toInt());
    this->setNetNumber(json.value("NetNumber").toString("0").toInt());
    this->setXYRegisterFormat(this->XYFormatStringToIndex(json.value("XYFormat").toString("1")));
    this->setProtocalType(this->MCProtocalStringToIndex(json.value("ProtocalType").toString("0")));
...
void CMDDevice::dynamicSetParameter(const QVariantMap &deviceParameterMap)
{
    ...
    for(int i=0;i<commParameterList.count();i++)
    {
        ...
        else if(0==QString::compare(parameterName,"NetNumber", Qt::CaseInsensitive))
        {
            this->setNetNumber((qint16)parameterValue.toInt());
        }
        else if(0==QString::compare(parameterName,"XYFormat", Qt::CaseInsensitive))
        {
            this->setXYRegisterFormat((bool)parameterValue.toInt());
        }
        else if(0==QString::compare(parameterName,"ProtocalType", Qt::CaseInsensitive))
        {
            this->setProtocalType((bool)parameterValue.toInt());
        }
...
QString CMDDevice::XYFormatIndexToString(bool index)
{
    QString strXYFormat= QString("%1").arg(index);
    if(index == 0)
    {
        strXYFormat = "Octal";
    }
...
bool CMDDevice::XYFormatStringToIndex(QString str)
{
    bool bXYFormat = (bool)str.toInt();
    if(0==QString::compare(str,"Octal", Qt::CaseInsensitive))
    {
        bXYFormat = 0;
    }
    else if(0==QString::compare(str,"Hex", Qt::CaseInsensitive))
    {
        bXYFormat = 1;
    }
    return bXYFormat;
}
QString CMDDevice::MCProtocalIndexToString(bool index)
{
    QString strMCProtocal= QString("%1").arg(index);
    if(index == 0)
    {
        strMCProtocal = "QnA-3E";
    }
    else if(index == 1)
    {
        strMCProtocal = "A-1E";
    }
    return strMCProtocal;
}
bool CMDDevice::MCProtocalStringToIndex(QString str)
{
    bool bMCProtocal = (bool)str.toInt();
    if(0==QString::compare(str,"QnA-3E", Qt::CaseInsensitive))
    {
        bMCProtocal = 0;
    }
    else if(0==QString::compare(str,"A-1E", Qt::CaseInsensitive))
    {
        bMCProtocal = 1;
    }
    return bMCProtocal;
...
void CMDDevice::initTranslateNameAndPropertyType()
{
    ...
    //!网络号
    m_translateName.insert(QString("NetNumber"),QString("网络号"));
    m_propertyTip.insert(QString("NetNumber"),QString("网络号"));
    //!XY寄存器地址格式
    m_translateName.insert(QString("XYRegisterFormat"),QString("XY寄存器地址格式"));
    m_propertyTip.insert(QString("XYRegisterFormat"),QString("XY寄存器地址格式"));
    //!通信协议
    m_translateName.insert(QString("ProtocolType"),QString("协议类型"));
    m_propertyTip.insert(QString("ProtocolType"),QString("协议类型"));
    //**********************************************************************//
...
//!站号
QString CMDDevice::station_Pro()
{
    return m_station_Pro;
}
void CMDDevice::setStation_Pro(QString station)
{
    m_station_Pro = station;
    m_station = (qint16)m_station_Pro.toInt();
}
//!通信协议
...
//!网络号
QString CMDDevice::netNumber_Pro()
{
    return m_netNumber_Pro;
}
void CMDDevice::setNetNumber_Pro(QString net)
{
    m_netNumber_Pro = net;
    m_netNumber = (qint16)m_netNumber_Pro.toInt();
}
//!XY寄存器地址格式
CMDDevice::MDXYFormat_Pro CMDDevice::xyRegisterFormat_Pro()
{
    return m_octalOrHex_Pro;
}
void CMDDevice::setXYRegisterFormat_Pro(CMDDevice::MDXYFormat_Pro format)
{
    m_octalOrHex_Pro = format;
    m_octalOrHex = (bool)m_octalOrHex_Pro;
}
//!通信协议
CMDDevice::MDMCProtocol_Pro CMDDevice::protocolType_Pro()
{
    return m_protocolType_Pro;
}
void CMDDevice::setProtocolType_Pro(CMDDevice::MDMCProtocol_Pro protocol)
{
    m_protocolType_Pro = protocol;
    m_protocalType = (bool)m_protocolType_Pro;
}
//***********************************************************************//
