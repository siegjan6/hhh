...
class CMDDevice:public CBaseDevice
{
    ...
    Q_PROPERTY(QString NetNumber READ netNumber_Pro WRITE setNetNumber_Pro)
    Q_ENUMS(MDXYFormat_Pro)
    Q_PROPERTY(MDXYFormat_Pro XYRegisterFormat READ xyRegisterFormat_Pro WRITE setXYRegisterFormat_Pro)
    Q_ENUMS(MDMCProtocol_Pro)
    Q_PROPERTY(MDMCProtocol_Pro ProtocolType READ protocolType_Pro WRITE setProtocolType_Pro)
    //*******************************************************************************//
public:
    CMDDevice();
    ~CMDDevice();
    ...
    //************************属性框枚举类-设备参数*************************//
    enum class  MDXYFormat_Pro
    {
        Octal = 0,
        Hex = 1,
    };
    enum class  MDMCProtocol_Pro
    {
        QnA_3E = 0,
        A_1E = 1,
    };
...
public:
    ...
    //!网络号
    QString netNumber_Pro();
    void setNetNumber_Pro(QString net);
    //!XY寄存器地址格式
    MDXYFormat_Pro xyRegisterFormat_Pro();
    void setXYRegisterFormat_Pro(MDXYFormat_Pro format);
    //!通信协议
    MDMCProtocol_Pro protocolType_Pro();
    void setProtocolType_Pro(MDMCProtocol_Pro protocol);
    //*******************************************************************//
private slots:
...
private:
    ...
    MDXYFormat_Pro m_octalOrHex_Pro;//!xy寄存器地址格式
    MDMCProtocol_Pro m_protocolType_Pro;//!协议类型
    QString m_netNumber_Pro;//!网络号
    //******************************************************************//
public:
   ...
   /*!
    ...
    /*!
     ...
     /*!
       * \brief setNetNumber:
          *  设置网络号.
       */
     void setNetNumber(qint16 netnumber);
     /*!
       * \brief xyRegisterFormat:
          *  XY寄存器地址格式.
       */
      bool xyRegisterFormat();
      /*!
        * \brief setXYRegisterFormat:
           *  设置XY寄存器地址格式.
        */
      void setXYRegisterFormat(bool format);
      /*!
        * \brief protocalType:
           *  通信协议.
        */
       bool protocalType();
       /*!
         * \brief setProtocalType:
            *  设置通信协议.
         */
       void setProtocalType(bool protocal);
    /*!
      * \brief configDecice:
         *  配置device界面.
      */
    ...
    QString XYFormatIndexToString(bool index);
    bool XYFormatStringToIndex(QString str);

    QString MCProtocalIndexToString(bool index);
    bool MCProtocalStringToIndex(QString str);
private:
    qint16 m_station;//!站号
    QString m_commParameter;//!通信参数
    CommunicationType m_commType;//!通信类型
    bool m_octalOrHex;//!xy寄存器地址格式
    bool m_protocalType;//!协议类型
    qint16 m_netNumber;//!网络号
