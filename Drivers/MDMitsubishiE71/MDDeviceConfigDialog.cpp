    ...
    ui(new Ui::CMDDeviceConfigDialog)
{
    ...
    ui->replyTimeoutEdit->setValidator(new QRegExpValidator(regExp2,this));
     ...
     ui->NetNumberEdit->setValidator(new QRegExpValidator(regExp2,this));
...
void CMDDeviceConfigDialog::setDevice(CMDDevice *device)
{
    ...
    if(m_device!=NULL)
    {
        ...
        ui->NetNumberEdit->setText(QString("%1").arg(m_device->netNumber()));
        if(m_device->xyRegisterFormat())
        {
            ui->octalRadioButton->setChecked(false);
            ui->hexRadioButton->setChecked(true);
        }
        else
        {
            ui->octalRadioButton->setChecked(true);
            ui->hexRadioButton->setChecked(false);
        }
        if(m_device->protocalType())
        {
            ui->QnA_3ERadioButton->setChecked(false);
            ui->A_1ERadioButton->setChecked(true);
        }
        else
        {
            ui->QnA_3ERadioButton->setChecked(true);
            ui->A_1ERadioButton->setChecked(false);
        }
...
void CMDDeviceConfigDialog::okButton_clicked()
{
    if(m_device!=NULL)
    {
        ...
        m_device->setNetNumber((qint16)ui->NetNumberEdit->text().toInt());
        if(ui->octalRadioButton->isChecked())
        {
            m_device->setXYRegisterFormat(false);
        }
        else
        {
            m_device->setXYRegisterFormat(true);
        }
        if(ui->QnA_3ERadioButton->isChecked())
        {
            m_device->setProtocalType(false);
        }
        else
        {
            m_device->setProtocalType(true);
        }
        //!更新属性框
