...
CMDDevice::CMDDevice()
{
    ...
    m_commParameter = "COM9000,115200,5,0,0";
    ...
    //***************************************************************//
    //************************属性框变量-设备参数***********************//
    m_commProtocol_Pro = "SiemensMPIProdave6";
    m_replyTimeout_Pro = "1000";
    m_retries_Pro = "3";
    m_mpiStationAddress_Pro = "2";
...
void CMDDevice::copy(IDevice*otherDevice)
{
    ...
    this->setMPIStation(((CMDDevice*)otherDevice)->MPIStation());
...
void CMDDevice::saveConfiguration(QDomDocument &doc,QDomElement &parentElement)
{
    ...
    element = doc.createElement("MPIStation");
    deviceElement.appendChild(element);
    elementText = doc.createTextNode(this->MPIStation());
    element.appendChild(elementText);


    element = doc.createElement("CPURack");
    deviceElement.appendChild(element);
    elementText = doc.createTextNode(QString("%1").arg(this->LocalTSAP()));
    element.appendChild(elementText);


    element = doc.createElement("CPUSlot");
    deviceElement.appendChild(element);
...
void CMDDevice::serialize(CSortJsonObject &json)
{
    ...
    json.insertSimpleValue("MPIStation",this->MPIStation());
    json.insertSimpleValue("CPURack",this->LocalTSAP());
    json.insertSimpleValue("CPUSlot",this->RemoteTSAP());
...
void CMDDevice::readConfiguration(QDomElement &domElement)
{
    ...
    while ( !element.isNull() )
    {
        ...
        else if(0==QString::compare(element.tagName(),"MPIStation", Qt::CaseInsensitive))
        {
            this->setMPIStation(element.text());
        }
        //****************************PKX*******************************//
        else if(0==QString::compare(element.tagName(),"CPURack", Qt::CaseInsensitive))
        {
            this->setLocalTSAP(element.text());
        }
        else if(0==QString::compare(element.tagName(),"CPUSlot", Qt::CaseInsensitive))
        {
...
void CMDDevice::deserialize(const QJsonObject &json)
{
    ...
    this->setCommParameter(this->commParameterStringToIndex(json.value("CommParameter").toString("COM9000,115200,5,0,0")));
    this->setReplyTimeout_Pro(json.value("ReplyTimeout").toString("1000"));
    this->setRetries_Pro(json.value("Retries").toString("3"));
    this->setMPIStation(json.value("MPIStation").toString("2"));
    this->setLocalTSAP(json.value("CPURack").toString("0"));
    this->setRemoteTSAP(json.value("CPUSlot").toString("2"));
...
void CMDDevice::dynamicSetParameter(const QVariantMap &deviceParameterMap)
{
    ...
    for(int i=0;i<commParameterList.count();i++)
    {
        ...
        else if(0==QString::compare(parameterName,"MPIStation", Qt::CaseInsensitive))
        {
            this->setMPIStation(parameterValue);
        }
        else if(0==QString::compare(parameterName,"CPURack", Qt::CaseInsensitive))
        {
            this->setLocalTSAP(parameterValue);
        }
        else if(0==QString::compare(parameterName,"CPUSlot", Qt::CaseInsensitive))
        {
...
void CMDDevice::setMPIStation(QString station)
{
    m_MPIStation = station;
    m_mpiStationAddress_Pro = m_MPIStation;
}
QString CMDDevice::MPIStation()
{
    return m_MPIStation;
}
qint8 CMDDevice::activeNO()
{
    return m_nActiveNO;
}
void CMDDevice::setActiveNO(qint8 num)
{
    m_nActiveNO = num;
}
...
void CMDDevice::initTranslateNameAndPropertyType()
{
    ...
    //!MPI站地址
    m_translateName.insert(QString("MPIStationAddress"),QString("MPI站地址"));
    m_propertyTip.insert(QString("MPIStationAddress"),QString("MPI站地址"));
    //!机架号
...
//!MPI站地址
QString CMDDevice::mpiStationAddress_Pro()
{
    return m_mpiStationAddress_Pro;
}
void CMDDevice::setMPIStationAddress_Pro(QString station)
{
    m_mpiStationAddress_Pro = station;
    m_MPIStation = m_mpiStationAddress_Pro;
}
