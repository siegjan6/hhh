    ...
    ui(new Ui::CMDDeviceConfigDialog)
{
   ...
   ui->portComboBox->addItem("COM9000");
...
void CMDDeviceConfigDialog::setDevice(CMDDevice *device)
{
    ...
    if(m_device!=NULL)
    {
        ...
        ui->LocalTSAP->setText(m_device->LocalTSAP());
        ui->RemoteTSAP->setText(m_device->RemoteTSAP());
        ui->MPIStationAddress->setText(m_device->MPIStation());
...
void CMDDeviceConfigDialog::okButton_clicked()
{
    if(m_device!=NULL)
    {
        ...
        m_device->setMPIStation(ui->MPIStationAddress->text());
