...
CMDDevice::CMDDevice()
{
    ...
    m_retries_Pro = "3";
    m_rackAndSlotOrTSAP_Pro = MDRackAndSlotOrTSAP_Pro::RackAndSlot;
    m_cpuRack_Pro = "0";
    m_cpuSlot_Pro = "2";
...
void CMDDevice::setLocalTSAP(QString LocalTSAP)
{
    ...
    if(m_rackAndSlotOrTSAP_Pro == MDRackAndSlotOrTSAP_Pro::RackAndSlot)
    {
...
{
    m_PLCType = PLCType;
    m_rackAndSlotOrTSAP_Pro = (MDRackAndSlotOrTSAP_Pro)m_PLCType;
}
...
//****************************PKX****************************//
    ...
    element = doc.createElement("TransportModel");
    deviceElement.appendChild(element);
    ...
    /*element = doc.createElement("LocalTSAP1");
    deviceElement.appendChild(element);
    elementText = doc.createTextNode(QString("%1").arg(this->LocalTSAP1()));
    element.appendChild(elementText);

    element = doc.createElement("LocalTSAP2");
    deviceElement.appendChild(element);
    elementText = doc.createTextNode(QString("%1").arg(this->LocalTSAP2()));
    element.appendChild(elementText);*/
    ...
    /*element = doc.createElement("RemoteTSAP1");
    deviceElement.appendChild(element);
    elementText = doc.createTextNode(QString("%1").arg(this->RemoteTSAP1()));
    element.appendChild(elementText);

    element = doc.createElement("RemoteTSAP2");
    deviceElement.appendChild(element);
    elementText = doc.createTextNode(QString("%1").arg(this->RemoteTSAP2()));
    element.appendChild(elementText);*/
//***********************************************************//
    if(this->getDataBlockCount()>0)
    {
...
void CMDDevice::serialize(CSortJsonObject &json)
{
    ...
    json.insertSimpleValue("TransportModel",this->transportModelIndexToString(this->PLCType()));
    json.insertSimpleValue("LocalTSAP",QString("%1").arg(this->LocalTSAP()));
    json.insertSimpleValue("RemoteTSAP",QString("%1").arg(this->RemoteTSAP()));
...
void CMDDevice::readConfiguration(QDomElement &domElement)
{
    ...
    while ( !element.isNull() )
    {
        ...
        //****************************PKX*******************************//
        else if(0==QString::compare(element.tagName(),"TransportModel", Qt::CaseInsensitive))
        {
        ...
        /*else if(0==QString::compare(element.tagName(),"LocalTSAP1", Qt::CaseInsensitive))
        {
            TempLocal = element.text() + ".";
        }
        else if(0==QString::compare(element.tagName(),"LocalTSAP2", Qt::CaseInsensitive))
        {
            TempLocal = TempLocal + element.text();
            this->setLocalTSAP(TempLocal);
        }
        else if(0==QString::compare(element.tagName(),"RemoteTSAP1", Qt::CaseInsensitive))
        {
            TempRemote = element.text() + ".";
        }
        else if(0==QString::compare(element.tagName(),"RemoteTSAP2", Qt::CaseInsensitive))
        {
            TempRemote = TempRemote + element.text();
            this->setRemoteTSAP(TempRemote);
        }*/
...
void CMDDevice::deserialize(const QJsonObject &json)
{
    ...
    this->setCommType(this->commTypeStringToEnum(json.value("CommType").toString("5")));
    this->setCommParameter(this->commParameterStringToIndex(json.value("CommParameter").toString("127.0.0.1")));
    this->setReplyTimeout_Pro(json.value("ReplyTimeout").toString("1000"));
    this->setRetries_Pro(json.value("Retries").toString("3"));
    this->setPLCType(this->transportModelStringToIndex(json.value("TransportModel").toString("0")));
    this->setLocalTSAP(json.value("LocalTSAP").toString("0"));
    this->setRemoteTSAP(json.value("RemoteTSAP").toString("2"));
...
void CMDDevice::dynamicSetParameter(const QVariantMap &deviceParameterMap)
{
    ...
    for(int i=0;i<commParameterList.count();i++)
    {
        ...
        else if(0==QString::compare(parameterName,"TransportModel", Qt::CaseInsensitive))
        {
...
TS7Client* CMDDevice::clientPoint()
{
    return m_pTS7Client;
}
...
QString CMDDevice::transportModelIndexToString(qint16 index)
{
    QString strTransportModel = QString("%1").arg(index);
    if(index == 0)
    {
        strTransportModel = "Rack/Slot";
    }
    else if(index == 1)
    {
        strTransportModel = "TSAP";
    }
    return strTransportModel;
}
qint16 CMDDevice::transportModelStringToIndex(QString str)
{
    qint16 nTransportModel = (qint16)str.toInt();
    if(0==QString::compare(str,"Rack/Slot",Qt::CaseInsensitive))
    {
        nTransportModel = 0;
    }
    else if(0==QString::compare(str,"TSAP",Qt::CaseInsensitive))
    {
        nTransportModel = 1;
    }
    return nTransportModel;
}
//**********************************属性框翻译与提示***************************//
...
//!rack or tsap
CMDDevice::MDRackAndSlotOrTSAP_Pro CMDDevice::rackAndSlotOrTSAP_Pro()
{
    return m_rackAndSlotOrTSAP_Pro;
}
void CMDDevice::setRackAndSlotOrTSAP_Pro(CMDDevice::MDRackAndSlotOrTSAP_Pro point)
{
    if(point!=m_rackAndSlotOrTSAP_Pro)
    {
        CPropertyBrowser* pProperty = getPropertyBrowse();
        if(point == MDRackAndSlotOrTSAP_Pro::RackAndSlot)
        {
            pProperty->setSinglePropertyHide("CPURack",false);
            pProperty->setSinglePropertyHide("CPUSlot",false);
            pProperty->setSinglePropertyHide("LocalTSAP",true);
        ...
        m_rackAndSlotOrTSAP_Pro = point;
        m_PLCType = (qint16)m_rackAndSlotOrTSAP_Pro;
    }
}
