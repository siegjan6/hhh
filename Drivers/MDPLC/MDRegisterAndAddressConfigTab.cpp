...
#include "ui_MDRegisterAndAddressConfigTab.h"
#include "MDDevice.h"
...
CMDRegisterAndAddressConfigTab::CMDRegisterAndAddressConfigTab(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::CMDRegisterAndAddressConfigTab)
{
    ui->setupUi(this);
    setWindowTitle(tr("寄存器与地址配置"));
    m_dataBlock = NULL;
    m_comboRow = -1;
    m_comboCol = -1;
    m_pLineEdit = NULL;
...
CMDRegisterAndAddressConfigTab::~CMDRegisterAndAddressConfigTab()
{
    delete ui;
}
void CMDRegisterAndAddressConfigTab::setDataBlock(CMDDataBlock *dataBlock)
{
...
void CMDRegisterAndAddressConfigTab::okButton_clicked()
{
    if(m_dataBlock== NULL)
    {
...
void CMDRegisterAndAddressConfigTab::cancelButton_clicked()
{
    this->setResult(QDialog::Rejected);
    this->reject ();
}
bool CMDRegisterAndAddressConfigTab::eventFilter(QObject *watched,QEvent *event)
{
...
void CMDRegisterAndAddressConfigTab::mousePressEvent(QMouseEvent *event)
{
    ui->tableWidget->removeCellWidget(m_comboRow,m_comboCol);
}

void CMDRegisterAndAddressConfigTab::tableWidget_cellDoubleClicked(int row, int column)
{
    if(column == 0)
    {
        if(m_comboRow !=row)
...
void CMDRegisterAndAddressConfigTab::setColString(QString str)
{
...
void CMDRegisterAndAddressConfigTab::addressEditFinish()
{
    if(m_pLineEdit==NULL)
    {
        ui->tableWidget->removeCellWidget(m_comboRow,m_comboCol);
...
void CMDRegisterAndAddressConfigTab::selectOtherItem()
{
    ui->tableWidget->removeCellWidget(m_comboRow,m_comboCol);
}
bool CMDRegisterAndAddressConfigTab::refreshBytesCount()
{
