...
CMDDevice::CMDDevice()
{
    ...
    m_commParameter = "Client,127.0.0.1,10025,,";
    m_station= 1;
    m_communicationProcessFlag = 0;
...
{
    this->setCommunicationProcessFlag(PROCESS_GETCONFIG);
...
void CMDDevice::deserialize(const QJsonObject &json)
{
    ...
    this->setCommParameter(this->commParameterStringToIndex(json.value("CommParameter").toString("Client,127.0.0.1,10025,,")));
    this->setReplyTimeout_Pro(json.value("ReplyTimeout").toString("1000"));
    this->setRetries_Pro(json.value("Retries").toString("3"));
    this->setStation(json.value("Station").toString("1").toInt());
...
void CMDDevice::setCommunicationProcessFlag(qint16 communication)
{
    m_communicationProcessFlag = communication;
}
qint16 CMDDevice::communicationProcessFlag()
{
    return m_communicationProcessFlag;
}
