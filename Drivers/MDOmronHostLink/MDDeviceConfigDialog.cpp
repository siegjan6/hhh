    ...
    ui(new Ui::CMDDeviceConfigDialog)
{
    ...
    //m_intValidator = new QIntValidator( 0, 255, this );
    //ui->stationEdit->setValidator(m_intValidator);
    //******************************PKX20151109******************************//
#ifdef Q_OS_LINUX
    ...
    if(m_device!=NULL)
    {
        if(m_device->commType()== CommunicationType::Serial)
        {
            ...
            if(paramList.size()>4)
            {
                 ...
                 ui->parityComboBox->setCurrentIndex(paramList.at(4).toUInt());//paramList.at(4).toUInt()//PKX
            }
        }
